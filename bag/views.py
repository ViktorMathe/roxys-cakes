...
def add_bag_content(request, cake_id):
    ...
    try:
        ...
        messages.error(
            request, 'Have to provide a number in the quantity \
                field to add the cake to the bag!')
        return redirect('cake_details', cake.id)
...
def change_bag(request, cake_id):
    ...
    try:
        ...
        if quantity > 0:
            ...
            bag.pop(cake_id)
            messages.warning(
                request, f'{cake.name} has been deleted from your bag')
    except Exception:
        bag = request.session.get('bag', {})
        bag.pop(cake_id)
        messages.error(request, f'{cake.name} has been removed from your bag!')
...
def delete_from_bag(request, cake_id):
    ...
    try:
        ...
        bag.pop(cake_id)
        messages.warning(
            request, f'{cake.name} has been removed from your bag')
